...
public interface CommentMapper {
    @Select("SELECT c.id, c.user_id AS userId, c.heritage_id AS heritageId, c.media_id AS mediaId, " +
            "c.content, c.status, c.create_time AS createTime, u.username " +
            "FROM heritage_comment c LEFT JOIN user u ON c.user_id = u.id " +
            "WHERE c.heritage_id = #{heritageId} ORDER BY c.create_time DESC")
    List<HeritageComment> selectByHeritageId(Long heritageId);

    @Select("SELECT c.id, c.user_id AS userId, c.heritage_id AS heritageId, c.media_id AS mediaId, " +
            "c.content, c.status, c.create_time AS createTime, u.username " +
            "FROM heritage_comment c LEFT JOIN user u ON c.user_id = u.id " +
            "WHERE c.media_id = #{mediaId} ORDER BY c.create_time DESC")
